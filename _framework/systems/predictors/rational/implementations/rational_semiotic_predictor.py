...
class RationalSemiotic(RationalPredictor):
    def __init__(self,
                 no_states: int, input_dimensions: int, output_dimensions: int, alpha: int, sigma: float,
                 drag: int, trace_length: int = 1, fix_level_size_at: Callable[[int], int] = lambda _level: -1):
        ...
        semiotic_keywords = {
            ...
            "drag": 1}
        self._predictor = SemioticModel[RATIONAL_INPUT, RATIONAL_INPUT](no_states, alpha, sigma, **semiotic_keywords)

    def __predict(self, data_in: Tuple[RATIONAL_INPUT, ...]) -> Tuple[RATIONAL_INPUT, ...]:
        return self._predictor.predict(data_in)

    def __fit(self, data_in: Tuple[RATIONAL_INPUT, ...], data_out: Tuple[RATIONAL_INPUT, ...]):
